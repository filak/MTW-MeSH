...
def getSparqlDataExt(dui, output, year="", key=None, cache=None, otype="Descriptor"):
    ...
    dview_ext_query = render_template(
        "sparql/descriptor_view.sparql",
        query=dui,
        official=True,
        year=year,
        otype=otype,
...
def updateSparqlBatch(
    ...
    try:
        with closing(
            requests.post(
                endpoint, headers=headers, data=sparql.encode("utf-8"), timeout=60
            )
        ) as r:
            stat = r.status_code
...
def parseSparqlData(data, sort_tree=None):
    ...
    for row in data["results"]["bindings"]:
        ...
        for item in row:
            val = row[item]["value"]
            val = (
                val.replace(app.config["SOURCE_NS"], "")
                .replace("vocab#", "")
...
def parseDescriptor(descriptor):
    ...
    for row in descriptor["results"]["bindings"]:
        ...
        if p == "preferredConcept":
            ...
            result["labels"]["prefCui"] = row["o"]["value"].replace(
                app.config["SOURCE_NS"], ""
            )
            ...
            if row.get("rrn"):
                details.append(
                    {
                        "p": "RN Related",
                        "val": row["rrn"].get("value", "").replace("~", "<br>"),
                    }
                )
            ...
            if p == "preferredTerm":
                result["labels"]["prefTerm"] = row["o"]["value"].replace(
                    app.config["SOURCE_NS"], ""
                )
            else:
                ui = row["o"]["value"]
